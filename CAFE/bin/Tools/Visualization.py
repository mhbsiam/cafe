...
from _plot_export import deferred_download_button
from embedding_explorer import render_embedding_explorer
from theme import TOKENS, apply_theme, info_card, page_header, section_header
from utils import (
    safe_flatten,
    safe_expression,
    safe_sort_clusters,
    get_cluster_col,
    compute_sample_proportions,
    compute_sample_frequencies_long,
    differential_abundance_test,
    plot_controls,
    RANDOM_STATE,
...
page_header(
    "Visualization",
    subtitle="UMAP, marker expression, clustering, and statistical plots with adjustable settings — all downloadable.",
)

uploaded_anndata = st.file_uploader(
    "Upload your AnnData (.h5ad) file", type=["h5ad"], accept_multiple_files=False
)

if uploaded_anndata is None:
    info_card(
        title="Load an AnnData object to begin",
        body="Upload the <code>.h5ad</code> file exported from Data Processing. "
             "Once loaded, you can explore clusters, compare groups, and download publication-ready figures.",
        kind="info",
    )
    st.stop()

if uploaded_anndata is not None:
    # Streamlit reruns this whole script on every widget interaction. Parsing the
    # uploaded .h5ad each time is the dominant cost on this page, so read it once
    # per uploaded file (keyed by Streamlit's per-upload file_id) and reuse the
    # in-memory AnnData on subsequent reruns.
    file_key = getattr(uploaded_anndata, "file_id", None) or (
        uploaded_anndata.name,
        uploaded_anndata.size,
    )
    if st.session_state.get("_viz_adata_key") != file_key:
        # read_h5ad is a single blocking call with no native progress hook, so
        # run it in a background thread and animate a progress bar on the main
        # thread while it works. The thread only touches the local dict below
        # (no Streamlit calls), so there's no ScriptRunContext issue.
        load_result = {}

        def _read_anndata():
            try:
                load_result["adata"] = ad.read_h5ad(uploaded_anndata)
            except Exception as exc:  # surface parse errors to the main thread
                load_result["error"] = exc

        reader = threading.Thread(target=_read_anndata)
        progress_bar = st.progress(0, text="Loading AnnData…")
        reader.start()
        pct = 0
        while reader.is_alive():
            # Creep toward 95% while the read runs; snap to 100% once it lands.
            pct = min(pct + 2, 95)
            progress_bar.progress(pct, text="Loading AnnData…")
            time.sleep(0.05)
        reader.join()
        progress_bar.progress(100, text="AnnData loaded")
        progress_bar.empty()

        if "error" in load_result:
            st.error(f"Failed to load AnnData: {load_result['error']}")
            st.stop()

        st.session_state["_viz_adata"] = load_result["adata"]
        st.session_state["_viz_adata_key"] = file_key
    adata = st.session_state["_viz_adata"]
    ...
    with st.expander("📑 Jump to section", expanded=False):
        st.markdown(
            ...
            "- [Overview](#overview)\n"
            "- [UMAP Analysis](#umap-analysis)\n"
            "- [Cells & Clusters](#cells-clusters)\n"
            "- [Sankey & Counts](#sankey-counts)\n"
            "- [Marker Profiles](#marker-profiles)\n"
            "- [Marker Distributions](#marker-distributions)\n"
            "- [Marker Comparisons](#marker-comparisons)\n"
            "- [Statistics](#statistics)"
        )

    # Interactive embedding explorer — surfaced right at the top for quick
    # access, outside UMAP Analysis. A live, zoomable WebGL view of the same
    # embedding; the static publication figures under UMAP Analysis are
    # unchanged. Wrapped in its own fragment so lasso/box selections (which
    # rerun on_select) re-run only this section, not the whole page.
    st.subheader("Interactive Explorer", anchor="interactive-explorer")
    st.caption("Pan, zoom, hover, and lasso-select cells to profile them. Recolor without losing "
               "your view. Use the static plots under **UMAP Analysis** for publication-ready downloads.")

    @st.fragment
    def render_interactive_explorer():
        render_embedding_explorer(st.session_state.adata, key_prefix="umap_explorer")
    ...
    unique_clusters = safe_sort_clusters(adata.obs[cluster_column].unique())

    st.subheader("Summary Tables", anchor="summary-tables")

    with st.form("data_form"):

        expression_method = st.radio("Select Expression Calculation Method", ('Mean', 'Median'))

        submit_button = st.form_submit_button(label='Generate Tables', type="primary")

    # Q3 fix: moved submit logic outside the form so output renders correctly
    if submit_button:
        cluster_list = []
        num_cells_list = []
        ...
        # B7 fix: use safe_expression which handles both sparse and dense matrices
        marker_names = adata.var_names
        expression_dict = {'Marker': marker_names}

        for group in unique_groups:
            group_expressions = [
                safe_expression(adata[adata.obs[group_column] == group][:, marker].X, expression_method)
        ...
        for cluster in unique_clusters:
            cluster_expressions = [
                safe_expression(adata[adata.obs[cluster_column] == cluster][:, marker].X, expression_method)
                for marker in marker_names
            ]
    ...
    if submit_button:
        ...
        for xyz in unique_clusters_1:
            ...
            for marker in markers:
                cluster_expression = safe_flatten(cluster_data[:, marker].X)
                marker_expression_value = safe_expression(cluster_data[:, marker].X, expression_method)
        ...
        sorted_cluster_keys = safe_sort_clusters(cluster_marker_dict.keys())
        ...
        clusters = safe_sort_clusters(adata.obs['leiden'].unique())
        ...
        deferred_download_button(fig, "pdf", "Matrix.pdf", "application/pdf", key_prefix="overview_matrix")
    ...
    st.subheader("UMAP Analysis", anchor="umap-analysis")

    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Clusters", "Marker Expression", "Selected Clusters", "Single Marker", "Per Sample"])
    ...
    def render_tab1():
        ...
        # Combined UMAP Plots
        st.caption("Combined UMAP plots colored by cell type / cluster identity.")
        with st.form(key='Plot1'):
            ...
            _c = plot_controls("umap_clusters", colormaps=cmap_option_20, default_cmap='tab20c')
            umap_width, umap_height, dot_size = _c.width, _c.height, _c.dot_size
            umap_cmap, umap_file_format = _c.cmap, _c.file_format

            submit_umap = st.form_submit_button("Plot UMAPs", type="primary")
    ...
    def render_tab2():
        ...
        st.caption("UMAP plots showing expression of each marker across all cells.")
        with st.form(key='Plot2'):
            ...
            _c = plot_controls(f"marker_expr{key_suffix}", include=("width", "height", "dot_size"),
                               width=(5, 20, 18), height=(5, 20, 14),
                               dot_size=(0.01, 20.0, 2.0), dot_size_step=0.01)
            marker_expr_width, marker_expr_height, marker_dot_size = _c.width, _c.height, _c.dot_size
            ...
            marker_expr_file_format = plot_controls(f"marker_expr_fmt{key_suffix}", include=("file_format",)).file_format
            submit_marker_expression = st.form_submit_button("Plot Marker Expression UMAPs", type="primary")
            ...
            if not markers_to_plot:
                ...
                if split_by_group == 'No':
                    ...
                    for i, marker in enumerate(markers_to_plot):
                        ax = axes[i]
                        marker_data = safe_flatten(adata[:, marker].X)
    ...
    def render_tab3():
        ...
        st.caption("UMAP highlighting selected clusters with optional marker overlay.")
        with st.form(key='Plot3'):
            ...
            umap_file_format = plot_controls("plot3_fmt", include=("file_format",)).file_format
            ...
            _c = plot_controls("plot3", include=("width", "height", "dot_size"))
            umap_width, umap_height, dot_size = _c.width, _c.height, _c.dot_size
            ...
            submit_umap = st.form_submit_button("Plot UMAP", type="primary")
        ...
        if submit_umap:
            ...
            if selected_marker not in adata_subset.var_names:
                ...
                marker_data = safe_flatten(adata_subset[:, selected_marker].X)  # Flattening sparse matrices if necessary
    ...
    def render_tab4():
        ...
        st.caption("UMAP comparing a single marker's expression between two selected groups.")
        with st.form(key='Plot4'):
            ...
            _c = plot_controls("plot4", include=("width", "height", "dot_size"))
            umap_width, umap_height, dot_size = _c.width, _c.height, _c.dot_size
    ...
    def render_tab5():
        ...
        st.caption("One UMAP plot per sample, colored by cluster identity.")
        with st.form(key='Plot5'):
            st.write("UMAP of each sample:")

            _c = plot_controls("sample_umap", include=("width", "height", "dot_size", "file_format"),
                               width=(1, 20, 5), height=(1, 20, 4), dot_size=(1, 100, 15),
                               file_formats=("PNG", "PDF", "SVG"))
            plot_width, plot_height, dot_size = _c.width, _c.height, _c.dot_size
            group_table_file_format = _c.file_format

            submit_frequency_table = st.form_submit_button("Plot UMAP of each sample", type="primary")
    ...
    # --- Function definitions for redistributed tabs (moved from Additional Plots) ---
    ...
    def render_tab19():
        ...
        st.caption("Compare central tendency of marker expression across three clusters, with ranked markers.")
        ...
        if submit_button:
            ...
            for marker in marker_names:
                if central_tendency == "Median":
                    expression_cluster_1.append(safe_expression(adata[adata.obs[cluster_col] == selected_cluster_1, marker].X, 'Median'))
                    expression_cluster_2.append(safe_expression(adata[adata.obs[cluster_col] == selected_cluster_2, marker].X, 'Median'))
                    expression_cluster_3.append(safe_expression(adata[adata.obs[cluster_col] == selected_cluster_3, marker].X, 'Median'))
                else:
                    expression_cluster_1.append(safe_expression(adata[adata.obs[cluster_col] == selected_cluster_1, marker].X, 'Mean'))
                    expression_cluster_2.append(safe_expression(adata[adata.obs[cluster_col] == selected_cluster_2, marker].X, 'Mean'))
                    expression_cluster_3.append(safe_expression(adata[adata.obs[cluster_col] == selected_cluster_3, marker].X, 'Mean'))
    ...
    def render_tab20():
        ...
        st.caption("Identify which clusters express a selected combination of markers above threshold.")
        with st.form(key='Plot_20'):
            st.write("Identify Clusters with Marker Combination")

            # B4 fix: expression_method was undefined (previously leaked from tab21)
            expression_method = st.radio('Select expression calculation method:', ('Mean', 'Median'))
        ...
        if submit_button:
            ...
            for xyz in unique_clusters_1:
                ...
                for i in sorted_idx:
                    marker_name = markers[i]
                    if marker_expressions[i] > MARKER_HIGH_THRESHOLD:
                        marker_name += "+"
                    elif marker_expressions[i] < MARKER_LOW_THRESHOLD:
                        marker_name += "-"
                    sorted_markers.append(marker_name)
    ...
    def render_tab21():
        ...
        st.caption("Bar plot of one cluster's expression across all markers.")
        with st.form(key='Plot_21'):
            st.write("One Cluster vs All Markers Expression:")

            expression_method = st.radio('Select expression calculation method:', ('Mean', 'Median'))

            # B3 fix: cluster_selected was undefined (previously leaked from another tab's scope)
            cluster_options = safe_sort_clusters(adata.obs['leiden'].unique())
            cluster_selected = st.selectbox("Select a cluster to compare", cluster_options)

            num_cols = st.slider("Select number of columns for subplots", min_value=1, max_value=10, value=4)
            _c = plot_controls("plot21", include=("width", "height"), size_unit="pixels",
                               width=(600, 1500, 1000), height=(400, 1200, 600))
            plot_width, plot_height = _c.width, _c.height

            bar_color = st.color_picker("Pick a color for the bars", "#1f77b4")

            submit_button = st.form_submit_button(label='Run Analysis', type="primary")
        ...
        if submit_button:
            ...
            for marker in markers:
                expr_selected.append(safe_expression(selected_cluster_data[:, marker].X, expression_method))
                expr_others.append(safe_expression(other_clusters_data[:, marker].X, expression_method))
            ...
            for idx, cluster in enumerate(unique_clusters):
                ...
                cluster_expr = [
                    safe_expression(cluster_data[:, marker].X, expression_method)
    ...
    def render_tab22():
        ...
        st.caption("Boxplot comparing per-sample cluster frequencies between two groups.")
        with st.form(key='Plot_22'):
            ...
            _c = plot_controls("plot22", include=("cmap", "file_format"),
                               colormaps=cmap_option_30, default_cmap='Set2')
            plot_cmap, plot_file_format = _c.cmap, _c.file_format
        ...
        if submit_button:
            ...
            freq_long = compute_sample_frequencies_long(adata, cluster_col)
            cell_counts = freq_long[freq_long['Group'].isin([group1, group2])]
            hue_order = [group1, group2]

            fig, ax = plt.subplots(figsize=(10, 6))
            sns.boxplot(
                x=cluster_col, y='frequency', hue='Group', data=cell_counts, palette=plot_cmap,
                hue_order=hue_order, ax=ax, dodge=True
            )
            # B5 fix: remove palette (conflicts with hue mapping in newer seaborn);
            # use a single black color for jitter points instead
            sns.stripplot(
                x=cluster_col, y='frequency', hue='Group', data=cell_counts,
                hue_order=hue_order, ax=ax, dodge=True, jitter=True,
                color='black', marker='o', size=3, linewidth=0.5, legend=False
            )
    ...
    def render_tab23():
        ...
        st.caption("Sankey / alluvial diagram showing cell flow between clusters and groups.")
        with st.form(key='Plot_23'):
            ...
            if 'leiden' in adata.obs.columns and 'Group' in adata.obs.columns:
                ...
                submit_sankey = st.form_submit_button(label='Generate Sankey Diagram', type="primary")
    ...
    def render_tab24():
        ...
        st.caption("Sankey diagram showing how cells above a marker threshold distribute across clusters.")
        with st.form("Plot_24"):
            ...
            _c = plot_controls("plot24", include=("width", "height"), size_unit="pixels",
                               width=(500, 1500, 1000), height=(300, 1200, 600))
            plot_width, plot_height = _c.width, _c.height
    ...
    def render_tab25():
        ...
        st.caption("2D kernel density estimate plot of two markers, optionally colored by a metadata column.")
        with st.form(key="Plot_25"):
            ...
            submit_button = st.form_submit_button(label="Generate KDE Plot", type="primary")
        ...
        if submit_button:
            ...
            try:
                x = safe_flatten(adata[:, marker1].X)
                y = safe_flatten(adata[:, marker2].X)
            except KeyError:
    ...
    def render_tab26():
        ...
        st.caption("Bar chart of total cell count in each cluster.")
        with st.form(key='Plot_26'):
            ...
            plot_file_format = plot_controls("cellvscluster_fmt", include=("file_format",), file_formats=("PNG", "PDF", "SVG")).file_format

            submit_plot = st.form_submit_button("Plot Cells vs Clusters", type="primary")
    ...
    def render_tab27():
        ...
        st.caption("Compare relative abundance of cells above mean for two markers between two clusters.")
        ...
        if submit_button:
            ...
            overall_mean_x = safe_expression(adata[:, marker_x].X, 'Mean')
            overall_mean_y = safe_expression(adata[:, marker_y].X, 'Mean')
            ...
            marker_x_expression_1 = safe_flatten(cluster_data_1[:, marker_x].X)
            marker_y_expression_1 = safe_flatten(cluster_data_1[:, marker_y].X)

            marker_x_expression_2 = safe_flatten(cluster_data_2[:, marker_x].X)
            marker_y_expression_2 = safe_flatten(cluster_data_2[:, marker_y].X)
    ...
    def render_tab28():
        ...
        st.caption("Correlation heatmap of selected markers.")
        with st.form(key='Plot_28'):
            ...
            correlation_matrix_plot_file_format = plot_controls("corrmatrix_fmt", include=("file_format",), file_formats=("PNG", "PDF", "SVG")).file_format
            show_numbers = st.checkbox("Show correlation values on the heatmap", value=True, key="show_numbers")
            submit_correlation_matrix_plot = st.form_submit_button("Plot Correlation Matrix", type="primary")
    ...
    def render_tab29():
        ...
        st.caption("Bar plot of cell count in a selected cluster across groups.")
        with st.form(key='Plot_29'):
            ...
            cellcount_plot_file_format = plot_controls("cellcount_fmt", include=("file_format",), file_formats=("PNG", "PDF", "SVG")).file_format

            submit_cell_count_plot = st.form_submit_button("Plot Cell Count", type="primary")
    ...
    def render_tab30():
        ...
        st.caption("Combined violin and bar plots of one marker's expression by a selected grouping.")
        with st.form(key='Plot_30'):
            ...
            violin_plot_file_format = plot_controls("violin_fmt", include=("file_format",), file_formats=("PNG", "PDF", "SVG")).file_format
            submit_violin_and_bar_plot = st.form_submit_button("Plot Violin and Bar Plots", type="primary")
        ...
        if submit_violin_and_bar_plot:
            ...
            bar_data = pd.DataFrame({group_by: adata.obs[group_by], 'expression': safe_flatten(adata[:, marker].X)})
    ...
    st.subheader("Cells & Clusters", anchor="cells-clusters")

    tab6, tab7, tab8, tab9 = st.tabs(["Cluster Heatmaps", "Composition Bars", "Cell % & Count", "Compare Two Clusters"])
    ...
    def render_tab6():
        ...
        st.caption("Heatmap of cell counts per cluster across groups.")
        with st.form(key='Plot6'):
            st.write("Heatmap Plot:")

            _c = plot_controls("heatmap", include=("width", "height", "cmap", "file_format"),
                               width=(1, 20, 8), height=(0.1, 5.0, 3.0),
                               colormaps=heatmap_cmap_options, default_cmap=heatmap_cmap_options[0])
            heatmap_width, heatmap_height = _c.width, _c.height
            heatmap_cmap, heatmap_file_format = _c.cmap, _c.file_format

            submit_combined_heatmap = st.form_submit_button("Plot Combined Heatmaps", type="primary")
    ...
    def render_tab7():
        ...
        st.caption("Stacked / grouped bar charts showing cluster composition within each group.")
        with st.form(key='Plot7'):
            ...
            bar_chart_file_format = plot_controls("barchart_fmt", include=("file_format",), file_formats=("SVG", "PDF")).file_format

            submit_combined_bar_chart = st.form_submit_button("Plot Combined Bar Charts", type="primary")
    ...
    def render_tab8():
        ...
        st.caption("Cell percentage and absolute count plots by group.")
        with st.form(key='Plot8'):
            st.write("Total Cell Percentage and Count Plots by Group:")

            _c = plot_controls("plot8", include=("width", "height", "file_format"),
                               width=(5, 20, 12), height=(1, 15, 3))
            plot_width, plot_height = _c.width, _c.height
            file_format = _c.file_format

            submit_cell_percentage_count = st.form_submit_button("Plot Cell Percentage and Count", type="primary")
    ...
    def render_tab9():
        ...
        st.caption("Compare relative marker abundance (cells above mean) between two clusters.")
        ...
        if submit_button:
            ...
            deferred_download_button(fig_comparison, "svg",
                f"Relative_Cells_{cluster_col.capitalize()}_{selected_cluster_1}_and_{selected_cluster_2}_{random_number}.svg",
                "image/svg+xml", key_prefix="tab9_comparison")
    ...
    st.subheader("Sankey & Counts", anchor="sankey-counts")
    tab23, tab24, tab26, tab29 = st.tabs(["Sankey: Cluster/Group", "Sankey: Marker Threshold", "Cells vs Clusters", "Count by Group"])
    ...
    st.subheader("Marker Profiles", anchor="marker-profiles")
    tab10, tab11, tab12, tab13, tab14, tab15 = st.tabs(["Dot Plot", "Heatmap", "Histogram", "Histogram: Group × Cluster", "Profile (Bar)", "Fold Change"])
    ...
    def render_tab10():
        ...
        st.caption("Dot plot of mean expression and fraction of expressing cells per marker per cluster, with optional dendrogram.")
        with st.form(key='Plot10'):
            st.write(f"All Markers vs All {cluster_col.capitalize()} Dot Plot:")

            _c = plot_controls("plot10", include=("width", "height"),
                               width=(1, 20, 10), height=(1, 20, 8))
            dotplot_width, dotplot_height = _c.width, _c.height
            ...
            dotplot_file_format = plot_controls("plot10_fmt", include=("file_format",)).file_format

            submit_dotplot_dendrogram = st.form_submit_button("Plot Dot Plot and Dendrogram", type="primary")
    ...
    def render_tab11():
        ...
        st.caption("Heatmap of mean / median marker expression across all clusters.")
        with st.form(key="Plot11"):
            ...
            submit_button = st.form_submit_button(label="Generate Heatmap", type="primary")
        ...
        if submit_button:
            ...
            random_number = random.randint(1000, 9999)
            deferred_download_button(fig, "svg",
                f"Matrixplot_{expression_method}_scaled_{cluster_col}_{random_number}.svg",
                "image/svg+xml", key_prefix="tab11_matrixplot")
    ...
    def render_tab12():
        ...
        st.caption("Histogram (KDE) of one marker's expression, faceted by a selected grouping column.")
        with st.form(key='Plot12'):
            ...
            with col9:
                kde_plot_file_format = plot_controls("kde_fmt", include=("file_format",)).file_format

            submit_kde_plot = st.form_submit_button("Plot Histogram", type="primary")
        ...
        if submit_kde_plot:
            ...
            for i, group in enumerate(unique_groups):
                group_data = safe_flatten(adata[adata.obs[group_by] == group, marker].X)
    ...
    def render_tab13():
        ...
        st.caption("Histogram (KDE) of one marker's expression, faceted by Group × Cluster grid.")
        with st.form(key='Plot_13'):
            ...
            with col7:
                kde_plot_file_format = plot_controls("kdegrp_fmt", include=("file_format",)).file_format
            submit_kde_plot = st.form_submit_button("Plot Histogram", type="primary")
        ...
        if submit_kde_plot:
            ...
            for group in unique_groups:
                for cluster in unique_clusters:
                    ...
                    group_cluster_data = safe_flatten(subset.X)
    ...
    def render_tab14():
        ...
        st.caption("Bar plot of one marker's mean / median expression across a selected grouping.")
        with st.form(key='Plot_14'):
            ...
            with col8:
                bar_plot_file_format = plot_controls("barprofile_fmt", include=("file_format",), file_formats=("SVG", "PDF")).file_format

            submit_bar_plot = st.form_submit_button("Plot Bar Plot", type="primary")
    ...
    def render_tab15():
        ...
        st.caption("Bar plot comparing one cluster's marker expression vs all others, with fold change and statistical test.")
        ...
        if submit_button:
            ...
            for marker in markers:
                fold_change = (
                    np.log2(safe_expression(selected_cluster_data[:, marker].X, expression_method) + 1) -
                    np.log2(safe_expression(other_clusters_data[:, marker].X, expression_method) + 1)
                )
    ...
    def get_expression_by_group_and_cluster(adata, selected_marker):
        ...
        for group in groups:
            for cluster in clusters:
                ...
                marker_values = safe_flatten(cluster_group_data[:, selected_marker].X)
    ...
    st.subheader("Marker Distributions", anchor="marker-distributions")
    tab16, tab17, tab18, tab25, tab27, tab28 = st.tabs(["Group Violin", "Bar (per Cluster)", "Bar (by Group)", "KDE", "Relative Abundance", "Correlation Matrix"])

    @st.fragment
    def render_tab16():
        adata = st.session_state.adata

        st.caption("Violin plot of one marker's expression split by group and cluster.")
        with st.form(key='Plot_16'):
            ...
            with col2:
                file_format = plot_controls("plot16_fmt", include=("file_format",), file_formats=("SVG", "PDF")).file_format

            submit_group_violin_plot = st.form_submit_button("Generate Group Violin Plot", type="primary")
        ...
        if submit_group_violin_plot:
            ...
            deferred_download_button(fig, file_format.lower(),
                f"ViolinPlot_{selected_marker}_{cluster_col}_{random_number}.{file_format.lower()}",
                f"image/{file_format.lower()}", key_prefix="tab16_violin")
    ...
    def render_tab17():
        ...
        st.caption("Bar plot comparing one selected cluster vs others across a combination of markers.")
        with st.form(key='Plot_17'):
            ...
            with col4:
                bar_plot_file_format = plot_controls("bar17_fmt", include=("file_format",), file_formats=("SVG", "PDF")).file_format
        ...
        if submit_bar_plot:
            ...
            _X = adata[:, selected_markers].X
            marker_expression_df = pd.DataFrame(_X.toarray() if issparse(_X) else np.asarray(_X), columns=selected_markers)
            ...
            deferred_download_button(bar_fig, bar_plot_file_format.lower(),
                f"BarPlot_{selected_cluster}_{cluster_col}_{random_number}.{bar_plot_file_format.lower()}",
                f"image/{bar_plot_file_format.lower()}", key_prefix="tab17_bar")
    ...
    def render_tab18():
        ...
        st.caption("Bar plot comparing marker expression across groups for a selected marker combination.")
        with st.form(key='Plot_18'):
            ...
            with col2:
                bar_plot_file_format = plot_controls("bar18_fmt", include=("file_format",), file_formats=("SVG", "PDF")).file_format
            ...
            submit_bar_plot_group = st.form_submit_button("Plot Bar Plot", type="primary")
        ...
        if submit_bar_plot_group:
            if 'Group' in adata.obs:
                ...
                deferred_download_button(bar_fig, "pdf",
                    f"BarPlot_GroupComparison_{random_number}.pdf",
                    "application/pdf", key_prefix="tab18_bar")
    ...
    st.subheader("Marker Comparisons", anchor="marker-comparisons")
    tab19, tab20, tab21, tab22, tab30 = st.tabs(["Compare 3 Clusters", "Find by Marker Combo", "Cluster vs Markers", "Cluster Frequency", "Violin + Bar"])
    ...
    st.subheader("Statistics", anchor="statistics")

    tab31, tab32, tab33, tab34, tab35, tab36 = st.tabs(["Group Comparison", "Marker-hi by Group", "Effect Size", "Differential Abundance", "Normality Test", "Group Correlation"])

    @st.fragment
    def render_tab31():
        adata = st.session_state.adata

        st.caption("Compare cluster abundance between two groups with boxplots and statistical tests.")
        with st.form(key="Plot_31"):
            ...
            _c = plot_controls("plot31", include=("width", "height"), size_unit="pixels",
                               width=(200, 1200, 600), height=(200, 1200, 600))
            plot_width, plot_height = _c.width, _c.height
            col5, col6 = st.columns(2)
            with col5:
            ...
            submit_comparison = st.form_submit_button(label="Compare Groups", type="primary")

        if submit_comparison:
            random_number = random.randint(1000, 9999)
            plot_buffers = []

            num_rows = max(1, (len(selected_comparisons) + num_columns - 1) // num_columns)
            fig = make_subplots(rows=num_rows, cols=num_columns, subplot_titles=selected_comparisons)

            cluster_col = 'cell_type' if 'cell_type' in adata.obs.columns else 'leiden'
            freq_long = compute_sample_frequencies_long(adata, cluster_col)
            freq_long = freq_long[freq_long['Group'].isin([group1, group2])]

            for idx, comparison in enumerate(selected_comparisons):
                comp_data = freq_long[freq_long[cluster_col] == str(comparison)]

                group1_data = comp_data[comp_data['Group'] == group1]['frequency']
                group2_data = comp_data[comp_data['Group'] == group2]['frequency']
    ...
    def render_tab32():
        ...
        st.caption("Compare marker-high cell proportions between groups for selected clusters and markers.")
        ...
        if submit_button:
            # B6 fix: validate that at least one marker was selected
            if not selected_markers:
                st.warning("Please select at least one marker before running the analysis.")
                st.stop()
            ...
            for idx, selected_marker in enumerate(selected_markers):
                _col_idx = filtered_adata.var_names.get_loc(selected_marker)
                _col_X = filtered_adata.X[:, _col_idx]
                df_filtered = pd.DataFrame(
                    _col_X.toarray() if issparse(_col_X) else np.asarray(_col_X),
    ...
    def render_tab33():
        ...
        st.caption("Effect size (Cohen's d or Cliff's Delta) of cluster frequency differences between two groups.")
        with st.form(key='Plot_33'):
            ...
            submit_button = st.form_submit_button(label='Calculate Effect Size', type="primary")

        if submit_button:
            random_number = random.randint(1000, 9999)
            plot_buffers = []

            freq_long = compute_sample_frequencies_long(adata, cluster_label)
            filtered_data = freq_long[freq_long['Group'].isin([group1, group2])]
            ...
            if filtered_data.empty:
                ...
                for cell_type in filtered_data[cluster_label].unique():
                    control_data = filtered_data[(filtered_data['Group'] == group1) &
                                                 (filtered_data[cluster_label] == cell_type)]['frequency']
                    treatment_data = filtered_data[(filtered_data['Group'] == group2) &
                                                   (filtered_data[cluster_label] == cell_type)]['frequency']
    ...
    def render_tab34():
        ...
        with st.form("Plot_34"):
            st.write("**Is there a statistically significant difference in cluster "
                     "abundance between groups? (per-sample differential abundance test)**")
            st.caption(
                "Each sample's cluster proportions are compared across groups "
                "(Mann-Whitney U for 2 groups, Kruskal-Wallis for >2), with "
                "Benjamini-Hochberg FDR correction across clusters. The sample "
                "— not the individual cell — is the unit of analysis."
            )

            submitted = st.form_submit_button("Run Differential Abundance Test", type="primary")
        ...
        if submitted:
            ...
            if n_groups < 2:
                st.warning("Need at least 2 groups to test differential abundance.")
                return
            if (group_counts < 2).any():
                small = ", ".join(f"{g} (n={n})" for g, n in group_counts.items() if n < 2)
                st.warning(
                    f"These groups have fewer than 2 samples: {small}. "
                    "Clusters cannot be tested for them and will show a NaN p-value."
                )

            st.write("**Samples per group used:** "
                     + ", ".join(f"{g}: {n}" for g, n in group_counts.items()))

            results_df = differential_abundance_test(prop_df, sample_group)

            n_sig = int(results_df['significant'].sum())
            n_total = int(results_df['p_adj'].notna().sum())
            st.write(
                f"**{n_sig} of {n_total} tested clusters** show significant "
                f"differential abundance (BH-adjusted p < 0.05)."
            )

            st.write("Per-cluster results (sorted by adjusted p-value):")
            st.dataframe(results_df, hide_index=True)

            # Heatmap: clusters x groups mean per-sample proportion.
            mean_cols = [c for c in results_df.columns if c.startswith('mean_prop_')]
            heatmap_df = results_df.set_index('cluster')[mean_cols]
            heatmap_df.columns = [c.replace('mean_prop_', '') for c in mean_cols]
            # Mark significant clusters with a trailing asterisk on the row label.
            sig_map = dict(zip(results_df['cluster'], results_df['significant'].fillna(False)))
            heatmap_df.index = [f"{c} *" if sig_map.get(c) else str(c) for c in heatmap_df.index]

            st.write("Heatmap of mean per-sample cluster proportion by group "
                     "(* = significant, BH-adjusted p < 0.05):")
            plt.figure(figsize=(max(4, 1.2 * heatmap_df.shape[1] + 2), max(3, 0.4 * heatmap_df.shape[0] + 1)))
            sns.heatmap(heatmap_df, annot=True, fmt=".3f", cmap="viridis",
                        cbar_kws={'label': 'Mean proportion'})
            plt.title("Mean per-sample cluster proportion by group")
            plt.ylabel("Cluster")
            plt.xlabel("Group")
            st.pyplot(plt.gcf())
            ...
            file_buffers.append(("DifferentialAbundanceResults", results_buffer))

            heatmap_buffer = io.BytesIO()
            plt.savefig(heatmap_buffer, format="png", dpi=300, bbox_inches='tight')
            heatmap_buffer.seek(0)
            file_buffers.append(("AbundanceHeatmap", heatmap_buffer))
            plt.close()

            st.download_button(
                label="Download Differential Abundance Results (CSV)",
                data=results_buffer.getvalue(),
                file_name=f"DifferentialAbundanceResults_{random_number}.csv",
                mime="text/csv",
            )

            st.download_button(
                label="Download Abundance Heatmap (PNG)",
                data=heatmap_buffer,
                file_name=f"AbundanceHeatmap_{random_number}.png",
                mime="image/png",
            )
            ...
            st.download_button(
                ...
                data=zip_buffer,
                file_name=f"DifferentialAbundanceAnalysis_{random_number}.zip",
                mime="application/zip",
            )
    ...
    def render_tab35():
        ...
        st.caption("Test whether marker expression is normally distributed within each group or cluster.")
    ...
    def render_tab36():
        ...
        st.caption("Clustered correlation heatmap of mean marker expression within each group, with dendrogram.")
        with st.form("Plot_36"):
            ...
            submitted = st.form_submit_button("Run Correlation and Dendrogram Analysis for Each Group", type="primary")
