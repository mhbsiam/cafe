...
# Reset to step 1 (method selection) whenever the user freshly navigates to this
# page. `_page_changed` is set by run.py/run_w.py and is True only on the first
# run after arrival — not on the reruns triggered by this page's own widgets, so
# step 2 stays open while the user fills it in.
...
def downsample_uniform(df, n_samples):
    # Every cell has equal probability -> preserves the original distribution.
...
def downsample_density_weighted(df, density, n_samples):
    # Sample proportional to density -> enriches dominant regions (original CAFE method).
...
def downsample_inverse_density(df, density, n_samples):
    # Sample inversely proportional to density -> favors sparse (rare) regions.
...
def process_file(file_path, output_dir, method, n_components, n_markers, n_samples):
    ...
    if len(df) <= n_samples:
        # Too few cells to downsample -> keep the file as-is and report it.
        downsampled_df = df
        skipped = True
    elif method == "uniform":
        downsampled_df = downsample_uniform(df, n_samples)
        skipped = False
    else:  # density_weighted or inverse_density -> both need PCA + KDE
...
# S1/S2: In web mode, filesystem access is disabled for security
if IS_WEB:
    st.warning(
    ...
    # Step 1: pick the downsampling method first. Everything else (directories,
    # sample count, cores) stays hidden until the user commits via "Continue",
    # so the form isn't overwhelming up front and the revealed controls always
    ...
    # Reveal the rest only after the method is confirmed. Stored in session
    # state so the options persist across the reruns caused by later widgets.
    if not st.session_state.get("downsampling_configured"):
        if st.button("Continue", type="primary"):
            st.session_state["downsampling_configured"] = True
            st.rerun()

    # Step 2: revealed only after a method has been chosen.
    if st.session_state.get("downsampling_configured"):
        ...
        # PCA/KDE controls apply to both density-based methods; uniform ignores them.
        ...
        if st.button("Apply", type="primary"):
            with st.spinner("The program is running...."):
                if input_directory and output_directory:
                    # S1 fix: validate paths against allowed base directory
                    try:
