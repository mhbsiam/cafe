...
parser.add_argument('--batch-map', dest='batch_map', default=None,
                    help='Optional CSV (columns: SampleID,Batch) mapping each sample to a '
                         'technical batch. When supplied, ComBat is run keyed on Batch while '
                         'preserving Group. Omit to skip batch correction.')
...
def perform_pca(adata, pca_solver, variance_threshold, output_dir):
    ...
    if not threshold_mask.any():
        print(f"Warning: No principal component reaches {variance_threshold}% variance. Using all {len(pca_variance_ratio)} components.")
...
def batch_correction(adata, batch_map_path):
    if not batch_map_path:
        print("Batch correction skipped (no --batch-map supplied).")
        return

    batch_df = pd.read_csv(batch_map_path)
    if 'SampleID' not in batch_df.columns or 'Batch' not in batch_df.columns:
        print("Batch correction skipped: --batch-map CSV must have 'SampleID' and 'Batch' columns.")
        return

    batch_map = dict(zip(batch_df['SampleID'].astype(str), batch_df['Batch'].astype(str)))
    ...
    if ok:
        print(f"{msg} (in {time.time() - start_time:.2f} seconds)")
    else:
        print(f"Batch correction skipped: {msg}")
...
def main():
    ...
    # Batch correction
    batch_correction(adata, args.batch_map)
