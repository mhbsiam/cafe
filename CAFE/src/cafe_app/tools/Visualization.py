...
from theme import IMG_DIR, apply_theme, info_card, page_header
from utils import (
    safe_flatten,
    safe_expression,
    safe_sort_clusters,
...
page_header(
    "Visualization",
    subtitle="UMAP, marker expression, clustering, and statistical plots with adjustable settings. All are downloadable.",
)

with st.form(key="upload_anndata_form"):
    uploaded_anndata = st.file_uploader(
        "Upload AnnData (.h5ad) file", type=["h5ad"], accept_multiple_files=False
    )
    submit_anndata = st.form_submit_button("Load AnnData", type="primary")

if submit_anndata and uploaded_anndata is not None:
    st.session_state["_viz_pending_upload"] = uploaded_anndata

pending_upload = st.session_state.get("_viz_pending_upload")

# Proceed if the user just uploaded a file OR data is already available (restored from disk after a
# refresh, or handed off from another page). Only block when there is genuinely nothing to show.
if pending_upload is None and st.session_state.get("adata") is None:
    info_card(
...
if pending_upload is not None:
    # A fresh upload: read it (skipping the re-read if the same file is already parsed) and persist so it
    # survives a refresh. set_adata mirrors the object into st.session_state.adata.
    file_key = getattr(pending_upload, "file_id", None) or (
        pending_upload.name,
        pending_upload.size,
    )
    if st.session_state.get("_viz_adata_key") != file_key:
        try:
            with st.spinner("Loading AnnData…"):
                adata = ad.read_h5ad(pending_upload)
        except Exception as exc:
            st.error(f"Failed to load AnnData: {exc}")
            st.stop()
        set_adata(adata, source="visualization", filename=pending_upload.name)
        st.session_state["_viz_adata_key"] = file_key
    adata = st.session_state["adata"]
    st.write(f"Loaded AnnData file: {pending_upload.name}")
else:
    # No new upload but data is present (restored from disk / another page).
    adata = st.session_state["adata"]
...
if "X_umap" not in adata.obsm:
    st.error(
        "This AnnData has no UMAP embedding (`X_umap` in `.obsm`), which the UMAP plots require. "
        "Re-export from CAFE Data Processing because the **UMAP + Leiden** step computes it, "
        "or compute a UMAP before uploading here."
    )
    st.stop()
...
# Interactive embedding explorer (surfaced at the top, wrapped in a fragment)
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Clusters", "Marker Expression", "Selected Clusters", "Single Marker", "Per Sample"], on_change="rerun")
...
def render_tab22():
    ...
    if submit_button:
        ...
        # Avoid palette conflict on jitter points
        sns.stripplot(
...
tab6, tab7, tab8, tab9 = st.tabs(["Cluster Heatmaps", "Composition Bars", "Cell % & Count", "Compare Two Clusters"], on_change="rerun")
...
tab23, tab24, tab26, tab29 = st.tabs(["Sankey: Cluster/Group", "Sankey: Marker Threshold", "Cells vs Clusters", "Count by Group"], on_change="rerun")
...
tab10, tab11, tab12, tab13, tab14, tab15 = st.tabs(["Dot Plot", "Heatmap", "Histogram", "Histogram: Group × Cluster", "Profile (Bar)", "Fold Change"], on_change="rerun")
...
tab16, tab17, tab18, tab25, tab27, tab28 = st.tabs(["Group Violin", "Bar (per Cluster)", "Bar (by Group)", "KDE", "Relative Abundance", "Correlation Matrix"], on_change="rerun")
...
tab19, tab20, tab21, tab22, tab30 = st.tabs(["Compare 3 Clusters", "Find by Marker Combo", "Cluster vs Markers", "Cluster Frequency", "Violin + Bar"], on_change="rerun")
...
tab31, tab32, tab33, tab34, tab35, tab36 = st.tabs(["Group Comparison", "Marker-hi by Group", "Effect Size", "Differential Abundance", "Normality Test", "Group Correlation"], on_change="rerun")
