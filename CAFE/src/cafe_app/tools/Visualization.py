...
from theme import IMG_DIR, TOKENS, apply_theme, info_card, page_header, section_header
from utils import (
    safe_flatten,
    safe_expression,
    safe_sort_clusters,
...
if st.session_state.get("_viz_pending_upload") is None:
    info_card(
...
uploaded_anndata = st.session_state["_viz_pending_upload"]

if True:
    # Cache AnnData parsing to avoid repeating the read on every rerun.
    ...
    if st.session_state.get("_viz_adata_key") != file_key:
        # Read AnnData in background thread with progress indicator.
        load_result = {}
